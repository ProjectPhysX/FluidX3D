...
#ifdef UTILITIES_CONSOLE_INPUT
// ASCII codes (key>0): 8 backspace, 9 tab, 10 newline, 27 escape, 127 delete, !"#$%&'()*+,-./0-9:;<=>?@A-Z[]^_`a-z{|}~¸‰ƒˆ÷‹ﬂµ¥ß∞π≥≤
...
// not working: π (251), num lock (-144), caps lock (-20), windows key (-91), kontext menu key (-93), F11 (-122)
#if defined(_WIN32)
...
#else // Linux
#include <termios.h>
inline int key_press() { // not working: π (251), num lock (-144), caps lock (-20), windows key (-91), kontext menu key (-93)
	struct termios term;
	tcgetattr(0, &term);
	while(true) {
		...
		int key = (int)getchar();
		if(key==27||key==194||key==195) { // escape, 194/195 is escape for ∞ﬂ¥‰ˆ¸ƒ÷‹
			key = (int)getchar();
		...
		switch(key) {
			...
			case -164: return 132; // ‰
			case -182: return 148; // ˆ
			case -188: return 129; // ¸
			case -132: return 142; // ƒ
			case -150: return 153; // ÷
			case -156: return 154; // ‹
			case -159: return 225; // ﬂ
			case -181: return 230; // µ
			case -167: return 245; // ß
			case -176: return 248; // ∞
			case -178: return 253; // ≤
			case -179: return 252; // ≥
			case -180: return 239; // ¥
